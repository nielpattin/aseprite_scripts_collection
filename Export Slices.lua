...
manager_dialog
    ...
    :button{ text = "Remove Selected Slices", onclick= function() 
...
function ReapplyAll()
    app.transaction( function()
        ...
        for i, slice in ipairs(sprite.slices) do
            ...
            then
                slice.color = manager_dialog.data.color
                slice.name = manager_dialog.data.name .. "_" .. no
                no = no + 1
            end
        end
...
function removeSelectedSlices() -- Remove selected Slices
    ...
    app.transaction( function()
        deleted = 0
        for n = 0, 10, 1 do
            for i, slice in ipairs(sprite.slices) do       --Check if this slice is inside the selection
